...
if not projectDir or not output_dir or not participant_id:
    parser.print_help()
    core.quit()


game_type = "NoiseRating"
...
if not re.search(pattern, participant_id):
...
outputFile = "%s/ratings_%s_%s.txt" % (output_dir, participant_id, expInfo['date'])
f = open(outputFile, 'w')
f.write("# Version: %s\n" % version)
    ...
    while continueRoutine:
        ...
        if currentSlide == instructionSlide and event.getKeys('space'):
            ...
            try:
                ...
                f.write("%s\t%s\t%d\n" %(participant_id, currentSound, rating))
                currentSlide = instructionSlide
